...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        header('Location: /register?success=1'); // Prevent form resubmission on refresh
        exit;
    } catch (Exception $e) {
        Logger::error("Registration error: " . $e->getMessage());
        $_SESSION['errors'][] = "Registration failed. Please try again.";
        header('Location: /register'); // Prevent form resubmission on refresh
        exit;
    }
}
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div id="lecturer-fields" style="display: <?= ($_POST['role'] ?? '') === 'lecturer' ? 'block' : 'none' ?>;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="course_code" name="course_code" placeholder="ITF12345" value="<?= htmlspecialchars($_POST['course_code'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
                </div>

                <div class="form-group">
                    <label for="course_name">Course Name</label>
                    <input type="text" id="course_name" name="course_name" placeholder="Datasikkerhet i utvikling og drift" value="<?= htmlspecialchars($_POST['course_name'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
                </div>

                <div class="form-group">
                    <label for="course_pin">Course PIN</label>
                    <input type="text" id="course_pin" name="course_pin" placeholder="1337" pattern="[0-9]{4}" title="Please enter a 4-digit PIN code" value="<?= htmlspecialchars($_POST['course_pin'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
                </div>
            </div>
